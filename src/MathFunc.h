...
T sinc_inv(const T x)
{
	...
	// We use the 4 order taylor series around 0 of x/sin(x) to compute
	// this function:
	//      2      4
	//     x    7⋅x     ⎛ 6⎞
	// 1 + ── + ──── + O⎝x ⎠
	//     6    360
	// this approximation is valide around 0.
	// if x is far away 0, our approximation is no more valid
	// since x^6 become non negligable we use the normal computation of the function
	...
	else
	{
		...
		{
			// x is upper the machine epsilon so x^2 is meaningful.
			T x2 = x*x;
