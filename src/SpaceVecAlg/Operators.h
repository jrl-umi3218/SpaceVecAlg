...
inline Eigen::Block<Derived, 3, Dynamic> motionAngular(Eigen::MatrixBase<Derived> & mv)
{
  return Eigen::Block<Derived, 3, Dynamic>(mv.derived(), 0, 0, 3, mv.cols());
}

template<typename Derived>
inline Eigen::Block<const Derived, 3, Dynamic> motionAngular(const Eigen::MatrixBase<Derived> & mv)
{
  return Eigen::Block<const Derived, 3, Dynamic>(mv.derived(), 0, 0, 3, mv.cols());
}

template<typename Derived>
inline Eigen::Block<Derived, 3, Dynamic> motionLinear(Eigen::MatrixBase<Derived> & mv)
{
  return Eigen::Block<Derived, 3, Dynamic>(mv.derived(), 3, 0, 3, mv.cols());
}

template<typename Derived>
inline Eigen::Block<const Derived, 3, Dynamic> motionLinear(const Eigen::MatrixBase<Derived> & mv)
{
  return Eigen::Block<const Derived, 3, Dynamic>(mv.derived(), 3, 0, 3, mv.cols());
}

template<typename Derived>
inline Eigen::Block<Derived, 3, Dynamic> forceCouple(Eigen::MatrixBase<Derived> & mv)
{
  return Eigen::Block<Derived, 3, Dynamic>(mv.derived(), 0, 0, 3, mv.cols());
}

template<typename Derived>
inline Eigen::Block<const Derived, 3, Dynamic> forceCouple(const Eigen::MatrixBase<Derived> & mv)
{
  return Eigen::Block<const Derived, 3, Dynamic>(mv.derived(), 0, 0, 3, mv.cols());
}

template<typename Derived>
inline Eigen::Block<Derived, 3, Dynamic> forceForce(Eigen::MatrixBase<Derived> & mv)
{
  return Eigen::Block<Derived, 3, Dynamic>(mv.derived(), 3, 0, 3, mv.cols());
}

template<typename Derived>
inline Eigen::Block<const Derived, 3, Dynamic> forceForce(const Eigen::MatrixBase<Derived> & mv)
{
  return Eigen::Block<const Derived, 3, Dynamic>(mv.derived(), 3, 0, 3, mv.cols());
}
...
inline RBInertia<T> PTransform<T>::dualMul(const RBInertia<T> & rbI) const
{
  ...
  I.template triangularView<Lower>() =
      E
      * (rbI.inertia() + vector3ToCrossMatrix(r) * vector3ToCrossMatrix<T>(rbI.momentum())
         + vector3ToCrossMatrix<T>(rbI.momentum() - rbI.mass() * r) * vector3ToCrossMatrix<T>(r))
      * E.transpose();
...
inline RBInertia<T> PTransform<T>::transMul(const RBInertia<T> & rbI) const
{
  ...
  I.template triangularView<Lower>() =
      ...
      - vector3ToCrossMatrix<T>(E.transpose() * rbI.momentum() + rbI.mass() * r) * vector3ToCrossMatrix<T>(r);
